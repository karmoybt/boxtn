// app/server/services/reservas.ts
...
export class ReservaService {
  ...
  async getReservas(filters: ReservaFilters = {}, userId: string) {
    ...
    return await this.repo.findBy(userId, filters);
  }
  ...
  async createReserva(
    ...
    const existing = await this.repo.findByUsuarioInstancia(userId, data.instancia_clase_id);
    if (existing) {
      throw new Error('Ya tienes una reserva para esta instancia de clase.');
    }

    // Estado inicial: 10 = confirmada
    const estado_id = 10;
  ...
  async updateReserva(
    ...
    const oldReserva = await this.repo.findById(id);
    if (!oldReserva) throw new Error('Reserva no encontrada.');

    await this.repo.update(id, { estado_id: data.estado_id });
