...
// Conexiones
const appDb = createClient({ url: 'file:./db.sqlite' })     // Tu app
const metaDb = createClient({ url: 'file:./genTurso.sqlite' }) // Config del generador
...
;[OUTPUT_DIR, REPO_DIR, INTERFACE_DIR].forEach(dir => {
...
async function main() {
  ...
  // Cargar excluidos y campos timestamp desde BD
  ...
  console.log(`⚙️  Excluyendo ${excludedTables.size} tablas. Reconociendo ${timestampFields.size} campos de fecha.`)

  // Obtener tablas de la app (db.sqlite)
  ...
  for (const tableName of tableNames) {
    ...
    // Zod schema con timestamp dinámico
    const zodFields = columns.map(col => {
      const { name, type, notnull } = col
      let zodType: string

      if (timestampFields.has(name)) {
...
export class ${typeName}RepositoryImpl implements ${typeName}Repository {
  ...
  async create( Create${typeName}Input): Promise<${typeName}> {
  ...
  async update(id: ${idType},  Update${typeName}Input): Promise<${typeName}> {
    const now = new Date().toISOString()
    const fields: string[] = []
    const args: unknown[] = []
    ${updateAssignments}
